defmodule Twirp.Client.HackneyTest do
  ...
  test "makes an http call if the rpc is defined", %{service: service} do
    Bypass.expect(service, fn conn ->
      ...
      body = Resp.encode(struct(Resp, msg: "test"))
    ...
    resp = Client.echo(struct(Req, msg: "test"))
    assert {:ok, struct(Resp, msg: "test")} == resp
  end
  ...
  test "json encoding and decoding", %{service: service} do
    ...
    assert {:ok, resp} = Client.echo(struct(Req, msg: "Test"))
  ...
  test "incorrect headers are returned", %{service: service} do
    ...
    assert {:error, resp} = Client.echo(struct(Req, msg: "test"))
  ...
  test "connect timeouts", %{service: _service} do
    assert {:error, resp} = Client.echo(%{connect_deadline: 0}, struct(Req, msg: "test"))
  ...
  test "recv timeouts", %{service: service} do
    ...
    assert {:error, resp} = Client.echo(%{deadline: 1}, struct(Req, msg: "test"))
    assert resp.code == :deadline_exceeded
    assert resp.meta["error_type"] == "timeout"
  end
  ...
  test "clients are easy to stub" do
    ...
    assert {:error, %Error{code: :unavailable}} = Client.echo(struct(Req, msg: "foo"))

    Twirp.Client.Stub.new(echo: fn %{msg: "foo"} ->
      struct(Resp, msg: "foo")
    end)
    assert {:ok, %Resp{msg: "foo"}} = Client.echo(struct(Req, msg: "foo"))

    assert_raise Twirp.Client.StubError, ~r/does not define/, fn ->
      Twirp.Client.Stub.new()
      Client.echo(struct(Req, msg: "foo"))
    end

    assert_raise Twirp.Client.StubError, ~r/expected to return/, fn ->
      Twirp.Client.Stub.new(echo: fn _ ->
        {:ok, struct(Req, msg: "test")}
      end)
      Client.echo(struct(Req, msg: "foo"))
    end
  end
